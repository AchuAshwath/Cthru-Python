...
Let's examine how CPython uses this metadata during program execution.

### The 16-Byte Header

Every object living on CPython's heap begins with the exact same 16-byte structural header:

- **`ob_refcnt` (8 bytes)**: The reference counter.
- **`ob_type` (8 bytes)**: A pointer pointing to the type descriptor struct (such as `PyLong_Type` or `PyList_Type`).
...
Inspect an object's reference count directly using `sys.getrefcount()`:
...
Here is how `PyObject` is defined and manipulated in CPython's C source code.
...
| :--------------- | :----------------------------------------------------------------------------------------------------------------- |
| **Basics**       | Numbers in Python are wrapped in an "identity badge" carrying type, count, and size metadata.                      |
| **Intermediate** | The 16-byte header enables automatic reference counting; small ints `[-5, 256]` are cached singletons.             |
| **Advanced**     | In C, `PyObject` is defined as `ob_refcnt` + `*ob_type`; integers use 30-bit digit arrays for arbitrary precision. |
